                  ...
                  {editingTeamIndex === index ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="rounded bg-sky-500 px-2 py-1 text-xs text-slate-950"
                      >
                        OK
                      </button>
                    </div>
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-950 px-4 text-slate-100">
      <div className="w-full max-w-3xl space-y-6 rounded-3xl bg-slate-900/80 p-8 shadow-2xl">
        ...
        <section className="space-y-3 rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
          ...
          <div className="space-y-3">
            <label className="flex cursor-pointer items-center gap-3">
              ...
              />
              <span className="text-sm text-slate-200">Классика — нельзя называть однокоренные слова</span>
            </label>
            <label className="flex cursor-pointer items-center gap-3">
              <input
                type="radio"
            ...
            <label className="flex cursor-pointer items-center gap-3">
              <input
                ...
                className="h-4 w-4 border-slate-600 bg-slate-900 text-sky-500"
              />
              <span className="text-sm text-slate-200">Шарады — ведущий отвечает «да»/«нет» на вопросы</span>
            </label>
          </div>
        </section>
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            {!user && (
              <p className="text-xs text-amber-400">Войдите в аккаунт.</p>
            )}
            <button
              type="button"
              onClick={handleCreateRoom}
              disabled={!user || !canGoNextFromCategories || (!gameSubModes.classic && !gameSubModes.gestures && !gameSubModes.charades)}
              className="rounded-lg bg-sky-400 px-4 py-2 text-sm font-semibold text-slate-950 shadow-lg shadow-sky-500/30 transition enabled:hover:bg-sky-300 disabled:cursor-not-allowed disabled:opacity-60"
            >
              Начать игру
            </button>
          </div>
